...
export default class Login extends Component {
    render() {
        ...
        //自定义校验  （密码）
        pwdValidator=(_,value="")=>{//想要第二个参数 第一个_占位 
            let errMsgArr=[]
            if(!value.trim()) return Promise.reject('密码必须输入')
            if(value.length<4) return errMsgArr.push('密码必须大于4位')
            if(value.length>12) return errMsgArr.push('密码必须小于12位')
